...
QuantConv= [-2.0794    -0.9163         0   0.9163   2.0794];%[-2.3026   -0.6931         0    0.6931    2.3026];
for i=1:length(LLRS)
datahold{datahold{:,9}==LLRS(i),9}=QuantConv(i);
datahold{datahold{:,16}==LLRS(i),16}=QuantConv(i);
end
%%
if wantChoiceOn==1
...
%General Settings:
Hs=unique(datahold{:,3});%Hs=[.05,.95,.5,.9];
ChoiceNoiseFlag=1;
...
Evidences=unique(ceil(datahold2.Sample_Angle));
...
if Hrate==1
    NumTrialsLLR(Active+2*(Hrate-1),i, Session)=sum(datahold2.Active_Angle==Choices(Active) & ceil(datahold2.Sample_Angle)==Evidences(i));
else
     prevAngle=datahold2.Active_Angle;
                      prevAngle=[NaN;prevAngle(1:end-1)];

   NumTrialsLLR(Active+2*(Hrate-1),i, Session)=sum(prevAngle==Choices(Active) & ceil(datahold2.Sample_Angle)==Evidences(i));
...
%     title({['Model threshold=   ', num2str(x(Session,2))]})%['Actual H=', num2str(H), ' NoiseChoice = ' num2str(NoiseProb), ' lapse= ' num2str(lapse)], 
    ...
    plot(bins,pChangetemp, colors{1+2*(Hrate-1)}, 'Linewidth', 3);
    hold on
    ...
    x(Session,:,Hrate)=fmincon(LogisticFit,[0,.5,0],[],[],[],[],[0,-4,0],[.3,4,10]);
        errorbar(bins,yfit(x(Session,:,Hrate)), errbartemp)%,  colors{2+2*(Hrate-1)}, 'Linewidth', 2');
        axis([-4,4,0,1]);
       
% 
      plot(bins,yfit(x(Session,:,Hrate)), colors{2+2*(Hrate-1)});
       legend('Data, H=.05','Logistic Fit, H=.05', 'Data, H=.95', 'Logistic Fit, H=.95','location','northwest');
                  set(gca,'FontSize',20)
...
% j=j+1;
             ...
             figure(figgy)%(2*(Session-1)+1)
             if Hrate==2
    title({['H=.05, Model Fit HRate=   ', num2str(z(1,1))],['H=.95, Model Fit HRate=   ', num2str(z(1,2))],[fileNAMES{Session}]})%['Actual H=', num2str(H), ' NoiseChoice = ' num2str(NoiseProb), ' lapse= ' num2str(lapse)], 
             else
                     title({['H=', num2str(Hs(Hrate)), 'Model Fit HRate=   ', num2str(z(1,1))]})%['Actual H=', num2str(H), ' NoiseChoice = ' num2str(NoiseProb), ' lapse= ' num2str(lapse)], 
